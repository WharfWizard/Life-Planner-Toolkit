// Show any runtime error in the page
...
// Wait until Recharts is available, then boot the app
(function bootWhenReady(tries = 0) {
  if (window.Recharts) {
    runApp();
  } else if (tries < 60) { // ~3s at 50ms intervals
    setTimeout(() => bootWhenReady(tries + 1), 50);
  } else {
    showErr('Recharts is not defined (CDN load failed).');
    console.error('window.Recharts:', window.Recharts);
  }
})();
                ...
                {flows.map((f)=>(
                  <tr key={f.id}>
                    ...
                    <td style={{textAlign:'right'}}>
                      <button onClick={()=>removeFlow(f.id)} style={{padding:'6px 10px', borderRadius:10, border:0, background:'#e2e8f0', color:'#0f172a', cursor:'pointer'}}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              return (
                <div key={p.id} style={{background:'#fff', borderRadius:16, border:'1px solid #e2e8f0', padding:12}}>
                  <div style={{display:'flex', gap:8, alignItems:'center', marginBottom:8}}>
                    ...
                    <button onClick={()=>removePot(p.id)} style={{padding:'6px 10px', borderRadius:10, border:0, background:'#e2e8f0', color:'#0f172a', cursor:'pointer', marginLeft:'auto'}}>Delete</button>
                  </div>
